...
import { abi as DMStaking } from "../abi/DMStaking.json";
...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const sapContract = new ethers.Contract(addresses[networkID].DM_ADDRESS as string, ierc20Abi, signer);
    const ssapContract = new ethers.Contract(addresses[networkID].SDM_ADDRESS as string, ierc20Abi, signer);
    let approveTx;
    let stakeAllowance = await sapContract.allowance(address, addresses[networkID].STAKING_HELPER_ADDRESS);
    let unstakeAllowance = await ssapContract.allowance(address, addresses[networkID].STAKING_ADDRESS);
    ...
    try {
      if (token === "sap") {
        ...
        approveTx = await sapContract.approve(
          addresses[networkID].STAKING_HELPER_ADDRESS,
          ethers.utils.parseUnits("1000000000", "gwei").toString(),
        );
      } else if (token === "ssap") {
        approveTx = await ssapContract.approve(
          addresses[networkID].STAKING_ADDRESS,
          ethers.utils.parseUnits("1000000000", "gwei").toString(),
        );
      }

      const text = "Approve " + (token === "sap" ? "Staking" : "Unstaking");
      const pendingTxnType = token === "sap" ? "approve_staking" : "approve_unstaking";
...
export const changeStake = createAsyncThunk(
  ...
  async ({ action, value, provider, address, networkID, callback }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    staking = new ethers.Contract(addresses[networkID].STAKING_ADDRESS as string, DMStaking, signer);
